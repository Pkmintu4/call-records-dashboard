...
def list_audio_files(access_token: str, folder_id: str | None = None) -> list[DriveFile]:
    ...
    while queue and len(seen) < max_folders:
        ...
        logger.info(f"Scanning folder {current_folder} for audio files...")
        candidates = _list_drive_files(access_token, query)

        for item in candidates:
            mime_type = (item.mime_type or "").lower()
            suffix = Path(item.name).suffix.lower()
            logger.info(f"Found file: name={item.name}, id={item.file_id}, mime_type={mime_type}, suffix={suffix}")
            if mime_type == FOLDER_MIME_TYPE:
                logger.info(f"  Skipping folder: {item.name}")
                if settings.drive_scan_recursive and item.file_id not in seen:
                    queue.append(item.file_id)
                continue

            if suffix in SUPPORTED_AUDIO_EXTENSIONS:
                logger.info(f"  Accepted by extension: {suffix}")
                results.append(item)
            elif mime_type.startswith("audio/"):
                logger.info(f"  Accepted by mime_type: {mime_type}")
                results.append(item)
            elif mime_type == "video/mp4":
                logger.info(f"  Accepted as video/mp4")
                results.append(item)
            else:
                logger.info(f"  Skipped: not audio or supported video")

    logger.info(f"Total audio files found: {len(results)}")
    return results
