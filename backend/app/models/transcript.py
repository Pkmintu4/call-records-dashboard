...
class Transcript(Base):
    ...
    transcription_rescue_used: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    transcription_rescue_provider: Mapped[str | None] = mapped_column(String(32), nullable=True)
    transcription_rescue_reason: Mapped[str | None] = mapped_column(String(64), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)

    sentiment = relationship("Sentiment", back_populates="transcript", uselist=False, cascade="all, delete-orphan")
    review = relationship("TranscriptReview", back_populates="transcript", uselist=False, cascade="all, delete-orphan")
