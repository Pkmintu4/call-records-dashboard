...
export function ingestNow({ folder, limit, forceReprocess, reprocessIgnoreOnly, audioOnly } = {}) {
  ...
  if (reprocessIgnoreOnly) {
    params.set("reprocess_ignore_only", "true");
  }
  if (audioOnly) {
...
export function getHumanReviewQueue(limit = 100, offset = 0) {
  const params = new URLSearchParams();
  params.set("limit", String(limit));
  params.set("offset", String(offset));
  return request(`/api/dashboard/transcripts/human-review-queue?${params.toString()}`);
}

export function submitHumanReview(transcriptId, payload) {
  return request(`/api/dashboard/transcripts/${transcriptId}/human-review`, {
    method: "POST",
...
export function getFrictionPoints(includeResolved = false) {
  const params = new URLSearchParams();
  if (includeResolved) {
    params.set("include_resolved", "true");
  }
  const query = params.toString();
  return request(query ? `/api/dashboard/friction-points?${query}` : "/api/dashboard/friction-points");
}

export function resolveFrictionPoint(transcriptId, frictionText) {
  return request(`/api/dashboard/friction-points/${transcriptId}/resolve`, {
    method: "POST",
    body: JSON.stringify({ friction_text: frictionText }),
  });
}

export function unresolveFrictionPoint(transcriptId, frictionText) {
  return request(`/api/dashboard/friction-points/${transcriptId}/unresolve`, {
    method: "POST",
    body: JSON.stringify({ friction_text: frictionText }),
  });
}

export function getFollowupCalls(includeResolved = false) {
  const params = new URLSearchParams();
  if (includeResolved) {
    params.set("include_resolved", "true");
  }
  const query = params.toString();
  return request(query ? `/api/dashboard/followup-calls?${query}` : "/api/dashboard/followup-calls");
}

export function resolveFollowupCall(transcriptId) {
  return request(`/api/dashboard/followup-calls/${transcriptId}/resolve`, {
    method: "POST",
  });
}

export function unresolveFollowupCall(transcriptId) {
  return request(`/api/dashboard/followup-calls/${transcriptId}/unresolve`, {
    method: "POST",
  });
}
