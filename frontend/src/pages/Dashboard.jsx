...
import FollowupCallsPanel from "../components/FollowupCallsPanel";
import FrictionPointsPanel from "../components/FrictionPointsPanel";
import SentimentDistributionChart from "../components/SentimentDistributionChart";
import SentimentTrendChart from "../components/SentimentTrendChart";
import TemporaryTranscriptViewer from "../components/TemporaryTranscriptViewer";
import HumanTranscriptReview from "../components/HumanTranscriptReview";
import {
  getCallDetail,
  getCallsByNumber,
  getCalls,
...
function Dashboard() {
  ...
  const [top10Calls, setTop10Calls] = useState([]);
  ...
  const [reprocessIgnoreOnly, setReprocessIgnoreOnly] = useState(false);
  ...
  const buildIngestSummary = (modeLabel, result) => (
    (() => {
      const reprocessMode = result.force_reprocess
        ? (result.reprocess_ignore_only ? "ignore-only" : "all-duplicates")
        : "off";
      return `${modeLabel} (${result.audio_only ? "audio-only" : "audio+txt"}): processed ${result.processed}, reprocessed ${result.reprocessed || 0}, attempted ${result.attempted}, skipped total ${result.skipped_total ?? result.skipped}, ` +
        `duplicate ${result.skipped_duplicate || 0}, reprocess-filter ${result.skipped_reprocess_filter || 0}, too short ${result.skipped_too_short || 0}, ` +
        `unsupported language ${result.skipped_unsupported_language || 0}, transcription errors ${result.failed_transcription || 0}, ` +
        `reprocess mode ${reprocessMode}, examined ${result.examined || 0}/${result.total_seen || 0}, folder ${result.folder_id || "env default"}. Showing top 10 calls in analytics.`;
    })()
  );
  ...
  const loadAll = async () => {
    const [trendData, distributionData, callsData, overallData, overallTrendData, segmentData, summaryRows, groupedRows, dbTableList, followupCallsData, frictionPointsData] = await Promise.all([
      getTrend(),
      getDistribution(),
      getCalls(),
      getOverallKpis(),
    ...
    setCalls(callsData);
    setTop10Calls(callsData.slice(0, 10));
    ...
    setFollowupData(followupCallsData);
    setFrictionData(frictionPointsData);
  };
  ...
  useEffect(() => {
    ...
    const savedReprocessIgnoreOnly = localStorage.getItem("driveReprocessIgnoreOnly");
    ...
    if (savedReprocessIgnoreOnly === "true") {
      setReprocessIgnoreOnly(true);
      setForceReprocess(true);
    }
  ...
  useEffect(() => {
    localStorage.setItem("driveReprocessIgnoreOnly", String(reprocessIgnoreOnly));
  }, [reprocessIgnoreOnly]);
  ...
  const handleSelectCall = async (id) => {
    ...
    try {
      ...
      setTimeout(() => {
        callDetailRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
      }, 100);
    } catch (err) {
  ...
  const latestBatchTranscripts = Array.isArray(latestBatchResult?.processed_records)
    ...
    : [];
  const temporaryViewerRows = latestBatchTranscripts.length > 0
    ? latestBatchTranscripts
    : transcriptSummaries;
    ...
    <div className="container dashboard-shell">
      ...
      <div className="header dashboard-header">
        ...
        <div className="dashboard-controls">
          ...
          <div
            className={`reprocess-status-badge ${reprocessIgnoreOnly ? "reprocess-status-on" : "reprocess-status-off"}`}
            title="Current reprocess scope"
          >
            Reprocess Scope: {reprocessIgnoreOnly ? "IGNORE only" : "All existing"}
          </div>
          <div
          ...
          <label className="force-reprocess-toggle">
            <input
              ...
              onChange={(event) => {
                ...
                setForceReprocess(checked);
                if (!checked) {
                  setReprocessIgnoreOnly(false);
                }
              }}
          ...
          <label className="force-reprocess-toggle">
            <input
              type="checkbox"
              checked={reprocessIgnoreOnly}
              onChange={(event) => {
                const checked = event.target.checked;
                setReprocessIgnoreOnly(checked);
                if (checked) {
                  setForceReprocess(true);
                }
              }}
              disabled={loadingIngest}
            />
            <span>Reprocess only IGNORE calls</span>
          </label>
          <label className="force-reprocess-toggle">
            <input
              type="checkbox"
      ...
      {/* Follow-up Calls Panel — placed prominently at the top */}
      <div className="kpi-grid-single">
        <FollowupCallsPanel
          data={followupData}
          ...
          onRefresh={async () => {
            try {
              const updated = await getFollowupCalls();
              setFollowupData(updated);
            } catch (err) {
              setError(err.message || "Failed to refresh follow-up calls");
            }
          }}
        />
      </div>

      {/* Friction Points Panel */}
      <div className="kpi-grid-single">
        <FrictionPointsPanel
          data={frictionData}
          onSelectCall={handleSelectCall}
          onRefresh={async () => {
            try {
              const updated = await getFrictionPoints();
              setFrictionData(updated);
            } catch (err) {
              setError(err.message || "Failed to refresh friction points");
            }
          }}
        />
      </div>
      ...
      <div className="grid" ref={callDetailRef}>
        <CallSentimentTable rows={top10Calls} onSelect={handleSelectCall} title="Top 10 Per-Call Sentiment Analytics" />
        <CallDetailPanel detail={selectedCall} />
      </div>

      {/* Removed All Calls Sentiment Analytics table as per user request. Only grouped table below. */}
      ...
      <div className="kpi-grid-single">
        <TemporaryTranscriptViewer rows={temporaryViewerRows} />
      </div>
